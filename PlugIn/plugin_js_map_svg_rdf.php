...
//
		...
		*/
   	  	...
   	  	if(type == 'onClick_event' && message.id == 'objectinfo')
  		{
			...
			$pfad);  
  			//,de.auster_gmbh.objectelement_toolbox.tools.functs.neutral
  		//alert('booh3');
  			//if(this.container[0].closed)
  			//{
  				//this.movetToPoint(this.container[0],0,0);
  				//this.container[0].transit(0,0);
  				//this.container[0].closed = false;
  				//this.container[1].transit(0,0);
  				//this.container[1].closed = false;
  			//}
  			//else
  			//{
  				//this.movetToPoint(this.container[0],0,0);
  				//this.container[1].transit(0,-1 * (window.innerHeight - 20));
  				//this.container[1].closed = false;
  				//this.container[0].transit((-1 * (window.innerWidth)) + 120,0);
  				//this.movetToPoint(this.container[0],(-1 * (window.innerWidth)) + 120,0);
		...
		*/
  	  	...
  	  	if(type == 'onClick_event' && message.id == 'okField')
  		{
			...
			request.addValue('id','1');
			request.addValue('ns',doc);
			request.addValue('URI',this.container[1]['InsertNode'].uri.getText());
			request.addValue('goto','0');
			request.addValue('attib','');
			request.addValue('stamp',stamp);
			request.addValue('newstamp','');
			request.addValue('success','true');
			*/
			
			de.auster_gmbh.library.access.execute_request(request);
			/*
			if(
			!(this.container[1]['InsertNode'].label == 'label' ||
			this.container[1]['InsertNode'].label == '')
			)
			
			  if(this.container[1]['InsertNode'].comment == '')
			  {
			    de.auster_gmbh.library.access.create_new_node(this.container[1]['InsertNode'].uri.getText()
			    , stamp, doc, this.container[1]['InsertNode'].label.getText() );
			  ...
			  else
			  {
			    de.auster_gmbh.library.access.create_new_node(this.container[1]['InsertNode'].uri.getText()
			    , stamp, doc, this.container[1]['InsertNode'].label.getText()
			    , this.container[1]['InsertNode'].comment.getText() );
		...
		if( type == 'selected_object' ) 
		{
  		...
  		if(type == 'init')
  		{
			...
			/*
			this.dataref.findIDX(1,'http://www.auster-gmbh.de/09/09/lib#door',0);
			//this.dataref.childNode(1,0);
			//alert(this.dataref.getID(1));
			var nodemany = this.dataref.childmany(1);
			var help = new Array();
			
			
			
			this.dataref.moveinWeb(1,'http://www.w3.org/2000/01/rdf-schema#label',0,true);
			var headline = this.dataref.curValue(1);
			this.dataref.moveinWeb(1,'http://www.w3.org/2000/01/rdf-schema#label',0,false);
			
			var myid = 0;
			var many = 0;
			for(it = 0;it < nodemany; it++)
			{
				this.dataref.childNode(1,it);
				
				help[it] = new Array();	
				
				myid = this.dataref.getID(1);
				many = this.dataref.childmany(1);
				if(this.dataref.moveinWeb(1,'http://www.w3.org/2000/01/rdf-schema#isDefinedBy',0,true))
				{
				
					if(many == 0)
					{
					help[it][1] = 'fincre_' + myid + '_1' ;
					}
					else
					{
					help[it][1] = 'create_' + myid + '_1' ;
					}
				...
				else
				{
					if(many == 0)
					{
					help[it][1] = 'finpos_' + myid + '_1' ;
					}
					else
					{
					help[it][1] = 'posnum_' + myid + '_1' ;
					}
				}
				
				this.dataref.findID(1,myid);
				
				
				this.dataref.moveinWeb(1,'http://www.w3.org/2000/01/rdf-schema#label',0,true);
				help[it][0] = this.dataref.curValue(1);
				this.dataref.moveinWeb(1,'http://www.w3.org/2000/01/rdf-schema#label',0,false);
 				
				this.dataref.parentNode(1,0);
				//alert(this.dataref.curNode(1));
