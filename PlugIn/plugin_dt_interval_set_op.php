		...
		for($i=0;$i < count($this->rst) ;$i++)
		{
			...
			do{
					...
					if(!is_array($result[$name][$from_str])
						...
						$result[$name][$from_str] = array('break', $this->inner_id, $hash);
				
					if(!is_array($result[$name][$to_str])
						$result[$name][$to_str] = array('stop', $this->inner_id++, $hash, $to_grid_str);
					else
						$result[$name][$from_str] = array('break', $this->inner_id, $hash);
						*/
			...
			foreach ($result as $key => $value)
				...
				{
					echo "table:" . $key . " No:" . $key2 . " " . $value2[0] . " " . $value2[1] . " " . $value2[2];
