...
//	echo count($arr) . " und $len rest $rest ";
	...
	*/
	...
	for($i = $start; $stop > $i;$i++)$vector[$i - $start] = intval($arr[$i]); 

//var_dump($vector);

	$res = array();

		for($i = 0;count($this->matrix) > $i; $i++)
		{
			$arr[$i + $start] = 0;			

			for($j = 0; count($this->matrix[$i]) > $j;$j++)
			{

				$arr[$i + $start] = ($arr[$i + $start] + ($this->matrix[$i][$j] * $vector[$j]) ) % $this->modul;
			}
...
//var_dump($res);
	...
	public function setMatrix($mat)
	{
		...
		$matrix = explode(';',trim($mat));
		for($i = 0;count($matrix) > $i;$i++)$this->matrix[$i] = explode(',',$matrix[$i]);
