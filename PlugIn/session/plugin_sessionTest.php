 ...
 * @covers Session
 */
class plugin_sessionTest extends TestCase
{      
	
	
    protected function setUp(): void
    {
        // Leere Session-Superglobal vor jedem Test
        $_SESSION = [];

        // Falls dein Code session_start() erwartet, kannst du das hier simulieren:
        if (session_status() === PHP_SESSION_NONE) {
            // Session Cookie vermeiden:
            session_start();
            // Und direkt auf CLOSED setzen, damit keine Headers mehr kommen
            session_write_close();
        }
    }
    ...
    public function testSessionInitialisation($testData):void
    {
        //$spacer='----------------------------------------------';
        ...
        $this->assertNotSame('no element received',$session->set_list($stubRst),"set_list should get dataobject (RST)");
        $this->assertSame('no element received',$session->set_list($testData),"set_list should object getting anything not an object");
        
        return;
    }

    #[DataProviderExternal(ExternalDataProvider::class, 'PlugInTestDataProvider')]
    public function testSessionWithRST($testData): void
    {           
        ...
        $stubRst = new RstStubForTesting($testData); // Pass the test data
        $session = new Session();
        $session->set_list($stubRst);
        // 5. Execute test logic and check results (Assertions)
        //    Test public functions 
        
        //col($columnName), where $columName != $tag_name, also moveFirst() and next() 
        //Row 1:
        $this->assertTrue($session->moveFirst(), "moveFirst should succeed");
        $this->assertSame('A',$session->col('grp'),"should give back A (testdata: 'grp' row1)");
        $this->assertSame(42,$session->col('counter'),"should give back A (testdata: 'grp' row1)");
        
        //Row 2:
        $this->assertTrue($session->next(), "next should succeed");
        $this->assertSame('B',$session->col('grp'),"should give back B (testdata: 'grp' row2)");
        $this->assertSame(42,$session->col('counter'),"should give back A (testdata: 'grp' row1)");
        
        //Row 3:
        $this->assertTrue($session->next(), "next should succeed");
        $this->assertSame('A',$session->col('grp'),"should give back A (testdata: 'grp' row3)");
        $this->assertSame(42,$session->col('counter'),"should give back A (testdata: 'grp' row1)");
        ...
        $this->assertSame(42,$session->col('counter'),"should give back A (testdata: 'grp' row1)");
        //End data:
        $this->assertFalse($session->next(), "next should fail");
        
        
    }
    
     public function testSessionWithoutRST(): void
    {           
        ...
        $this->assertTrue($session->moveFirst(), "moveFirst should succeed");
        $this->assertSame(42,$session->col('counter'),"should give back A (testdata: 'grp' row1)");
        $this->assertSame('Ralf',$session->col('name'),"should give back A (testdata: 'grp' row1)");
