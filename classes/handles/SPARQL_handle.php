...
class SPARQL_handle extends Interface_handle 
{
...
//var_dump($out);
				...
				//var_dump($json_array['head']['vars']);
				$vars = $json_array['head']['vars'];
				$ns_set = [];
				//var_dump($json_array['results']['bindings']);
				
				foreach($json_array['results']['bindings'] as $trible)
					foreach($vars as $var)
						//var_dump($trible, $var);
						if($trible[$var]['type'] == 'uri')
						{
							$ns_parts = explode('#', $trible[$var]['value']);
